...
`npm run perf:seed -- --source <db> --out <dir>` creates a scrubbed database for performance measurements, while `dev:seed` copies data and files for interactive development. It defaults to the installed app database and `~/.convergence-perf-seed/convergence.db`, takes one `VACUUM INTO` snapshot through a read-only handle (excluding deleted source pages), blanks known secret fields, masks recognized token strings and secret-keyed JSON strings without changing their stored text length, and verifies every text value before publishing. A per-run in-memory map keeps distinct matches distinct; verification recognizes only that run's issued fillers. JSON numbers, booleans and null remain unchanged. Secure deletion, final compaction and WAL truncation remove obsolete bytes, including old index separators. Attachments and session outputs are omitted. This is not general anonymization. Only Marcin or Fable runs it on real data; executors always supply a generated temporary fixture with `--source`. Close the target before reseeding; errors report only the phase, table and SQLite code, delete the unpublished copy and preserve any previous output.
