...
import type { RemoteCodeReviewDaemonConnectionResult } from '@/entities/code-review-guide'
import { Button } from '@/shared/ui/button'
import { Input } from '@/shared/ui/input'
import { cn } from '@/shared/lib/cn.pure'
import { SettingsControlField } from './settings-control-field.presentational'

interface GuidedReviewGenerationFieldsProps {
  backend: GuidedReviewBackend
  remoteBaseUrlDraft: string
  remoteBaseUrlError: string | null
  credentialStatus: GuidedReviewDaemonCredentialStatus | null
  ...
  connectionResult: RemoteCodeReviewDaemonConnectionResult | null
...
function credentialStatusText(
  status: GuidedReviewDaemonCredentialStatus | null,
): string {
...
export const GuidedReviewGenerationFields: FC<
  GuidedReviewGenerationFieldsProps
> = ({
  backend,
  remoteBaseUrlDraft,
  remoteBaseUrlError,
  ...
  <div className="space-y-4">
    <SettingsControlField
      ...
      description="Choose whether guided review generation runs inside Convergence or through a remote agents daemon."
    >
      <div className="grid grid-cols-2 gap-2" role="group">
        {(['local', 'remote'] as const).map((option) => (
          <Button
            key={option}
            type="button"
            variant={backend === option ? 'secondary' : 'outline'}
            size="sm"
            aria-pressed={backend === option}
            onClick={() => onBackendChange(option)}
          >
            {option === 'local' ? 'Local' : 'Remote daemon'}
          </Button>
        ))}
      </div>
    </SettingsControlField>
    ...
    <SettingsControlField
      ...
      description="Base URL for the remote agents daemon. The token is stored separately in Keychain."
    >
      <div className="space-y-2">
        <label
          htmlFor="guided-review-daemon-base-url"
          className="text-xs font-medium text-muted-foreground"
        >
          Base URL
        </label>
        <Input
          id="guided-review-daemon-base-url"
          ...
          aria-describedby={
            remoteBaseUrlError
              ? 'guided-review-daemon-base-url-error'
              : undefined
          }
        />
        {remoteBaseUrlError && (
          <p
            id="guided-review-daemon-base-url-error"
    ...
    <section className="rounded-xl border border-border bg-card/45 p-4">
      ...
      <div className="mt-4 space-y-2">
        <label
          htmlFor="guided-review-daemon-token"
          className="text-xs font-medium text-muted-foreground"
        >
          Token
        </label>
        <div className="flex gap-2">
          <div className="relative min-w-0 flex-1">
            ...
            <Button
              ...
              aria-label={
                showDaemonToken ? 'Hide daemon token' : 'Show daemon token'
              }
