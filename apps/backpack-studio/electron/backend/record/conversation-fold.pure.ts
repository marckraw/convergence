...
export interface ConversationFold {
   ...
   *
   * Describes the local record only. A lost start reply can record `refused`
   * even though the daemon created the session; routing must ask the daemon,
   * never infer remote existence from this fact (RUN50 R6).
   */
