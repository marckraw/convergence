 ...
 */
const renames = vi.fn()
let healingFixture: string | null = null
let healReads = 0
vi.mock('node:fs/promises', async (importOriginal) => {
  ...
  return {
    ...
    readFile: (...args: Parameters<typeof real.readFile>) => {
      if (healingFixture !== null && String(args[0]).endsWith('events.jsonl')) {
        healReads++
        return Promise.resolve(healingFixture)
      }
...
describe('JsonFileConversationStore', () => {
  // R6 L1 mutation: bypass per-id chain -> two heals append two newline boundaries.
  it('serializes concurrent first appends including the heal', async () => {
    await store.create(record())
    const initial = line(wire(envelope(1, 'first')))
    await writeFile(join(root, 'c-1', 'events.jsonl'), initial)
    healingFixture = initial
    healReads = 0
    try {
      await Promise.all([
        store.appendEntry('c-1', wire(envelope(2, 'second'))),
        store.appendEntry('c-1', wire(envelope(3, 'third'))),
      ])
    } finally {
      healingFixture = null
    }
    expect(healReads).toBe(1)
    expect(await store.readLog('c-1')).toMatchObject({
      unreadableTailLines: 0,
      entries: [
        wire(envelope(1, 'first')),
        wire(envelope(2, 'second')),
        wire(envelope(3, 'third')),
      ],
    })
  })
...
describe('a torn log tail', () => {
  ...
  /**
   * Mutation: delete the `healLog` call from `appendEntry` and the two
