...
export function statusBadge(status: ConversationStatus): StatusBadge {
  switch (status) {
    case 'running':
      return { label: 'Working', tone: 'var(--studio-explore-accent)' }
    case 'idle':
      return { label: 'Ready for you', tone: 'var(--studio-explore-connected)' }
    case 'failed':
      return { label: 'Failed', tone: 'var(--studio-explore-ink)' }
  }
}
...
/** Lists hold only the five summary fields, never transcript items. */
export function conversationSummary(
  row: ConversationSummary,
): ConversationSummary {
  const { id, title, createdAt, updatedAt, status } = row
  return { id, title, createdAt, updatedAt, status }
}
