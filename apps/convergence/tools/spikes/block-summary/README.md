...
`electron/backend/block-sentence/block-sentence.prompt.txt` (moved there by MAR-3395 CV3, which ships it) is the exact common prompt; only provider and tool records are appended. Fixture labels and truth sets are withheld. Claude uses pretty JSON inputs; Pi compact JSON; Cursor free text; Codex results only with commands as tool names. Truth paths include literal paths, their parent directories and basenames, plus literal listing entries. The generator is deterministic and derives each truth set with the app's own `deriveBlockTruth`; `truthCheck`, `LUNA_MODEL_ID` and `LUNA_ONE_SHOT_TIMEOUT_MS` are likewise imported back from `electron/backend/block-sentence/block-sentence.pure.ts`, so the spike and the app gate the same way. Both candidates use greedy decoding and a 96-token response limit. Apple additionally uses a `@Generable` structure with one `sentence` field; MLX emits plain text. This decoding difference is intentional, not an identical-model comparison.
