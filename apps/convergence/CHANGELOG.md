...
- 649919b: The sidebar's activity feed shows Review before Working, marks Pinned as special, and every section folds while still showing what is inside.
...
- f888f2e: A conversation that has a crew seat but is not the crew's mastermind now shows "Run the drill" greyed out in the context popover, with one sentence saying where to set the seat's role (the crew's settings in Mission Control). Before, the button just wasn't there. The popover also checks again every time you open it, so a role you just changed shows up right away, without waiting for a turn to end.

## 0.80.2

### Patch Changes

- 4da871c: While a conversation compacts its context, every place that names its state now says "Compacting context…" instead of "Finished": the Needs-you card (under Working, with no Acknowledge), the header (one pill, not two), the status bar, the sidebar and Mission Control badges, and the command center. A message you send while it compacts, or while the drill runs, is no longer refused: it waits in the queue ("Waits for compaction") and is sent once the compaction or the drill is done. The composer tells you this before you type. While the drill runs, the header shows its stage: `drill · sealing`, `drill · compacting`, `drill · resuming`.
...
- 977929b: A Cursor conversation keeps its process across turns, stops cleanly with a real cancel instead of ending failed, queues what you type mid-turn like the other providers, gives a silent turn a ten-minute budget, and recovers from a stale resume token. Manual context compaction is temporarily unavailable for Cursor while the conversation keeps its process across turns.
...
- 556dac9: The wave column's Open button explains itself when the window is too narrow, and the panel's clock only runs when something on screen shows an age.

## 0.65.1

### Patch Changes

- 66372c1: A Cursor conversation whose local record cannot be written mid-turn no longer dies or reports a failed send: the loss is noted on the conversation and the run continues to its end.
...
- c4f1113: A crew can be bound to a Linear project; the app keeps a read-only ledger of the issues labeled for each seat, joined with the seat's session, PR and host facts.
...
- d4434c0: A crew member is a seat: role, kind, role card, host and lane policy, WIP limit; one seat belongs to one crew. A seat's role card leads the first message a wire carries into it in each run, and is not repeated on later laps. A conversation that sits in two crews keeps the earliest membership — the later one is removed when the app next opens, unless a wire in that crew is still armed on it.
...
- ce37266: Crew import offers the member already holding a role's baton instead of leaving
...
- e394517: The refusal a remote session gets when it has nowhere to work is written once
  (MAR-2999).

  The sentence that turns away a remote session with no stated work place lived in
  two files, byte for byte: `REMOTE_SPAWN_PLACE_REQUIRED` in the shared spawn
  spec, which the composer draft, the recipe normalizer and the crew YAML reader
  all ask, and a literal of its own inside the session service's create door. Two
  encodings of one fact only ever agree until someone edits one of them, and this
  is exactly the kind of sentence that gets edited — a user reports the wording,
  the wording changes in the place it was found, and the other half of the app
  goes on saying the old thing with nothing anywhere to notice.

  The create door now imports the shared constant. No refusal changed and no
  session's fate changed; the two doors simply cannot drift apart any more. A test
  puts both of them against that one constant rather than against a quoted string,
  so what it pins is not the wording but that there is only one of it.
...
- 3ead3ef: Annotations waiting to be sent now sit in one compact row instead of stacking up the screen.

  When you answer several places in a long reply, each annotation used to take a full-width row above the composer, so fourteen of them pushed the conversation out of view. They now wait as small pills in a single row that scrolls sideways, with a count of how many there are, and the composer stays its normal height.

  Click a pill (or focus it and press Enter) to open it and read, edit or remove that annotation; press Escape to close it again. While you are editing, the first Escape throws away the unsaved change and keeps the annotation open, and a second Escape closes it. The row is a single Tab stop — arrow keys, Home and End move along it, and inside the edit field they move the cursor as usual. What gets sent is unchanged — every annotation, in the same order, with the same words.

## 0.58.3

### Patch Changes

- 129b7e5: Simplify Activity filters with All activity, Needs me, Working, and Review views plus host and provider icon toggles, contextual counts, tooltips, and one-click reset. Remove the advanced filter panel and reset legacy saved filters so every project remains visible.
...
- d1d52cf: Show provider logos in session cards, the composer, model pickers, and provider settings. Keep model names easy to scan and provider names accessible in both themes (MAR-3000).
  ...
  Add a collapsible activity toolbar with search, metadata filters, visible result counts, removable selections, grouping, sorting, and section ordering. Explain empty results and hidden pins, remember the view, and hold ordering during card interaction.

  Show linked PR numbers and review states below the model. Show trustworthy elapsed turn or background-task time beside the status, freeze completed durations, and leave unknown timing unspecified.

- d1d52cf: Replace project conversation rows with compact provider-tinted cards that expand when selected. Keep existing project actions, inline rename, workspace grouping and terminal rows, while showing provider, model and host/type identities at a glance.
...
- 46a1b73: A relay delivery to a session that is mid-turn now waits instead of failing.
...
- b154ed0: The Needs You clock runs while the feed has cards, and refreshes the moment one
  arrives (MAR-2994).
...
- c53c5fb: The tests that walk a source tree spend one named budget (MAR-2989).
...
- 4a54bcf: Queued follow-ups survive the turn they waited behind, and a failed one can be delivered again.
...
- 8409f15: The run's hop backstop now follows the crew's delivery limit (MAR-2966).
...
- 5381840: Convergence shows the parallel work of a Claude Code session: a side panel of agents, commands and monitors with their own transcripts, markers where they start and return, and a session that reads 'answered · N tasks running' while work continues.
...
- 0c1f856: Codex hygiene: no recovery note after `/clear`, a warming-up usage pill, and one thread per first message
  ...
  - The composer's Codex usage pill reads "warming up" while the resident app-server is starting, instead of a number it can no longer stand behind, and reads the limits of the account this session would actually send on (MAR-2825).
  - A session sent a second message before its first thread exists now ends up with one thread on the server rather than two — including when that thread is being started by a recovery or by a `/clear` (MAR-2826).
  - Clearing a conversation that has not started one no longer leaves an unused thread behind for the next launch to warn about (MAR-2826).
...
- 962bcbb: Lanes, slice L1 — the lane exists (MAR-2783). A project can now spawn a lane:
...
- eca5541: The baton loop: a crew can close its own loop safely (MAR-2759). A relay may
...
- 4a53be8: The Claude Code model picker knows Claude Fable 5.1: a pinned `claude-fable-5-1` entry (1M context, effort low through max) joins the catalog at the top of the pinned list, and the Best/Fable aliases now name Fable 5.1 as the model they resolve to. Fable 5 stays available as a pinned legacy entry. (MAR-2768)

## 0.46.7

### Patch Changes

- a82c68c: Extract the daemon client core into `@convergence/execution-host-client`
...
- b003a27: A remote session says which branch it works on, and the daemon says back which one it made (MAR-2694, MAR-2718).
...
- 8a98548: A remote session now says where it works before you send it (MAR-2689).
...
- 1212ac3: The option row above the Execution Bar stops claiming things about a remote
...
- 0572ff9: Settings holds many execution host endpoints (MAR-2642, MAR-2629).
...
- b96e6cb: Sessions record which machine they ran on (MAR-2620).
...
- bd6a0f4: Convergence is the conversation and Mission Control (MAR-2609).
...
- 1a9b49a: A working session card breathes in its crew's colour (MAR-2607).
...
- 4150753: A file change's identity now includes the repository it lives in (MAR-2589).
...
- 1672b22: The session header stops saying "Running" about sessions that are not (MAR-2590).
...
- 84f06e9: A truncated diff stops rendering as if it were whole (MAR-2577).
...
- bcdbde2: The two React hook lint rules finally run on this renderer (MAR-2545).
...
- 024ca73: Remote sessions stop being one-shot (MAR-2582).
...
- ac84592: The remote execution host speaks the daemon's real contract (MAR-2576).
...
- 3a851ec: Change the model mid-conversation, and a transcript that says which one wrote
