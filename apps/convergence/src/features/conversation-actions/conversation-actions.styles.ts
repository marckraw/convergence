...
export const conversationActionsStyles = {
  ...
  fan: 'absolute bottom-0 right-0 h-0 w-0',
  fanItem: `absolute ${PILL} ${FOCUS_RING} ${POP}`,
  fanClose: `absolute bottom-0 right-0 ${PILL} w-24 px-0 ${FOCUS_RING} ${POP}`,
  panel: `absolute z-40 flex flex-col overflow-hidden rounded-2xl border border-border/80 bg-popover/95 text-popover-foreground shadow-xl backdrop-blur-xl ${POP}`,
